...
function NormalPlayer(props) {
  ...
  const _getPosAndScale = () => {
    const trargetWidth = 40;
    const paddingLeft = 40;
    const paddingBottom = 30;
    const paddingTop = 80;
    const width = window.innerWidth * 0.8;
    const scale = trargetWidth / width;
  ...
  const enter = () => {
    ...
    const { x, y, scale } = _getPosAndScale();//获取miniPlayer唱片中心的偏移
    let animation = {
      0: {
        transform: `translate3d(${x}px,${y}px,0) scale(${scale})`
      },
      60: {
        transform: `translate3d(0,0,0) scle(1.1)`
      },
      100: {
        transform: `translate3d(0,0,0) scle(1)`
      }
    };
